...
public class BatchDao {
	...
	public int updateBatchDetails(batch batch) {
		...
		try {
			Connection con = DbResource.getDbConnection();
			PreparedStatement pstmt = con.prepareStatement("update batch set typeofbatch = ? where batchid = ?");
			pstmt.setInt(2, batch.getBatchid());
			pstmt.setString(1, batch.getTypeofbatch());
			return pstmt.executeUpdate();
		} catch (Exception e) {
			System.err.println("Batch update error "+e.toString());
			return 0;
	...
	public int deleteBatchDetails(int batchid) {
		
		try {
			Connection con = DbResource.getDbConnection();
			PreparedStatement pstmt = con.prepareStatement("delete from employee where batchid = ?");
			pstmt.setInt(1, batchid);
			return pstmt.executeUpdate();
		} catch (Exception e) {
			System.err.println("Batch delete error "+e.toString());
			return 0;
